...
describe('manifest — rules-only fixtures (schema-valid, §2-invalid)', () => {
  it.each(fixtures('examples/invalid-rules', MANIFEST_SUFFIX))('%s passes schema but breaks a §2 rule', (file) => {
    const doc = loadJson('examples/invalid-rules', file) as { name: string }
    expect(validateManifest(doc), JSON.stringify(validateManifest.errors, null, 2)).toBe(true)
    expect(doc.name).not.toBe(file.slice(0, -MANIFEST_SUFFIX.length))
  })
})
