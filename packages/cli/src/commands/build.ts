...
export async function cmdBuild(args: string[], ctx: CommandContext): Promise<CommandResult> {
  ...
  for (const name of names) {
    ...
    if (ctx.dryRun) {
      ...
      ctx.ui.out(ctx.ui.style('dim', `would write: ${name} page${flags.has('--lock') ? ' and lock' : ''} (${plural(manifest.data.skills.length, 'member')})`))
    ...
    ctx.ui.out(
      `${ctx.ui.style('green', '✓')} ${name} — ${page} (${plural(manifest.data.skills.length, 'member')}${flags.has('--lock') ? ', locked' : ''})`,
    )
    sets.push({ name, page, members: manifest.data.skills.length, locked: flags.has('--lock') })
  }
